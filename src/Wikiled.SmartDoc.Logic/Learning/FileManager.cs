...
namespace Wikiled.SmartDoc.Logic.Learning
{
    public class FileManager : IFileManager
    {
        ...
        public async Task<DocumentSet> LoadAll(DirectoryInfo repositoryPath)
        {
            var allFiles = await Task.Run(
                ...
                Directory.EnumerateFiles(repositoryPath.FullName, "*", SearchOption.AllDirectories)
                         .Where(item => string.Compare(new FileInfo(item).Directory.FullName, repositoryPath.FullName, StringComparison.OrdinalIgnoreCase) != 0) // don't take root files
                         .ToArray(),
                token);
            ...
            try
            {
                ...
                input.Complete();
                await Task.WhenAll(chain.Item1.Completion, chain.Item2.Completion);
            }
            catch (Exception ex)
            {
        ...
        private async Task<DocumentDefinition> ParseDocument(DirectoryInfo repositoryPath, string file)
        {
            try
            {
                return await parser.ParseDocument(repositoryPath, new FileInfo(file), token);
            }
            catch (Exception ex)
            {
                logger.Error(ex);
